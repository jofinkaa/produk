...
<div class="mb-10 flex justify-between items-center">
    <h2 class="text-3xl font-black text-gray-900 uppercase italic tracking-tighter">
        Edit <span class="text-[#8B0000]">FAQ Kategori Produk</span>
    </h2>
...
<div class="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 max-w-2xl mx-auto">
    <form action="{{ route('admin.faq.update', $faq->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="question" value="{{ old('question', $faq->question) }}" class="w-full px-5 py-3 rounded-xl border border-stone-200 focus:outline-none focus:border-black font-medium text-sm" required>
        </div>

        {{-- INPUT KATEGORI (PENTING: Perlu pengecekan kategori yang sudah terpilih) --}}
        @php
            $selectedCategories = explode(',', $faq->category);
        @endphp
        <div>
            <label class="block text-[10px] font-black uppercase tracking-wider text-gray-400 mb-2">Kategori (Knowledge Tagging)</label>
            <div class="flex flex-wrap gap-3">
                @foreach(['Tas', 'Kain', 'Tenun', 'Aksesoris', 'Pajangan', 'Umum'] as $cat)
        ...
        <div>
            ...
            <textarea name="answer" rows="6" class="w-full px-5 py-3 rounded-xl border border-stone-200 focus:outline-none focus:border-black font-medium text-sm text-stone-700 leading-relaxed" required>{{ old('answer', $faq->answer) }}</textarea>
        </div>
